...
function getWeekendsInCycle(cycleYear: number): { year: number; month: number; week: number; dates: string[] }[] {
  ...
  const cycleMonths = [
    ...
    { y: cycleYear, m: 5 }, { y: cycleYear, m: 6 }, { y: cycleYear, m: 7 }, // Jun, Jul, Aug
    { y: cycleYear, m: 8 }, { y: cycleYear, m: 9 }, { y: cycleYear, m: 10 },// Sep, Oct, Nov
    { y: cycleYear, m: 11 }, { y: cycleYear + 1, m: 0 }, { y: cycleYear + 1, m: 1 } // Dec, Jan, Feb
  ];
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (dow === 6) {
        ...
        if (currentWeekDates.length > 0 && lastSundayMonth !== m) {
          weekCount++;
          weekends.push({ year: y, month: m + 1, week: weekCount, dates: currentWeekDates });
          lastSundayMonth = m;
        }
        currentWeekDates = [];
      }
...
export function generateSchedule(
  ...
  for (let i = 0; i < shuffledBlocks.length; i++) {
    ...
    const resident: ResidentName = i % 2 === 0 ? "Luís Gustavo" : "Ana Carolina";
  ...
  // Group by month
  const weekendsByMonth: Record<number, { week: number; dates: string[] }[]> = {};
  ...
  for (let cycleIdx = 0; cycleIdx < monthsInCycle.length; cycleIdx++) {
    ...
    if (month === 3) {
      ...
      for (let wIdx = 0; wIdx < monthWeekends.length; wIdx++) {
        ...
        if (w.dates.length > 0) {
          if (!assigned.has(w.dates[0])) {
            ...
            results.push({ date: w.dates[0], resident: mainResident, shiftType: "weekend" });
          }
        }
        if (w.dates.length > 1) {
          if (!assigned.has(w.dates[1])) {
            assigned.add(w.dates[1]);
            results.push({ date: w.dates[1], resident: secondaryResident, shiftType: "weekend" });
          }
        }
      }
    } else {
      ...
      if (secondWeekend) {
        ...
        const resident: ResidentName = aaShifts <= mcShifts ? "Ana Amélia" : "Maria Cecília";
        
        for (const dateStr of secondWeekend.dates) {
          if (assigned.has(dateStr)) continue;
          assigned.add(dateStr);
          results.push({ date: dateStr, resident, shiftType: "weekend" });
        }
      }
  ...
  for (const dateStr of lgDates) {
    if (assigned.has(dateStr)) continue;
    assigned.add(dateStr);
    results.push({ date: dateStr, resident: "Luís Gustavo", shiftType: "weekend" });
  }
  for (const dateStr of acDates) {
    if (assigned.has(dateStr)) continue;
    assigned.add(dateStr);
    results.push({ date: dateStr, resident: "Ana Carolina", shiftType: "weekend" });
  }
